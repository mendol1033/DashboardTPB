...
class Monevcctvit_model extends CI_Model {
	...
	public function GetDataTable(){
		...
		if (!empty($_GET['tglAwal']) && !empty($_GET['tglAkhir'])) {
			$this->monev_db->where('Tanggal >=',$_GET['tglAwal']);
			$this->monev_db->where('Tanggal <=',$_GET['tglAkhir']);
		}
	...
	public function add(){
		...
		if(isset($_POST['activeCCTV'])){
			$statusCCTV = "Y";
		} else {
			$statusCCTV = "N";
		}

		if(isset($_POST['activeIT'])){
			$statusIT = "Y";
		} else {
			$statusIT = "N";
		}

		if(isset($_POST['realtimeCCTV'])){
			$realtimeCCTV = "Y";
		} else {
			$realtimeCCTV = "N";
		}

		if(isset($_POST['realtimeIT'])){
			$realtimeIT = "Y";
		} else {
			$realtimeIT = "N";
		}
		...
		$data = array(
			...
			'realtimeCCTV' => $realtimeCCTV,
			'statusIT' => $statusIT,
			'realtimeIT' => $realtimeIT,
...
/* End of file monevcctvit_model.php */
/* Location: ./application/models/hanggar/monevcctvit_model.php */