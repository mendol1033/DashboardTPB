<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTable').DataTable({
			...
			"order" : [],
			"ajax" : {
				"url" : "<?php echo base_url().'hanggar/monevcctvit/ajax_list'?>",
				"type" : "POST",
			},
		});
		...
		$("#mainFormCCTVIT").validate({
			...
			errorPlacement:function(error,element){
				...
				$("#mainFormCCTVIT > .form-group").addClass('has-error');
			}
		});
	});
	...
	function view(id){
		$.ajax({
			url: "<?php echo base_url().'hanggar/monevcctvit/ajax_edit';?>",
			type: "GET",
			dataType: "JSON",
			data: {id : id},
	...
	$("#btnTambah").on('click', function(event) {
		save_method = "add";
		event.preventDefault();
		$(".modal-title").text('Tambah Data Laporan Kondisi CCTV dan IT Invetory');
		$("#mainFormCCTVIT")[0].reset();
		$("#NamaPerusahaan > option").remove();
		$('[name="JnsDokumen"]').trigger('change');
		$("#mainFormCCTVIT").remove(".help-block");
		$("#mainFormCCTVIT > .form-group").removeClass('has-error');
		$("#modalFormCCTVIT").modal("show");
	...
	function save(){
		var url;
		var data;
		data = $("#mainFormCCTVIT").serializeArray();

		if(save_method == "add"){
			url = "<?php echo base_url().'hanggar/monevcctvit/ajax_add'?>";
		} else {
			data[data.length] = {name: "id", value: idEdit};
			url = "<?php echo base_url().'hanggar/monevcctvit/ajax_update'?>";
		}

		if($("#mainFormCCTVIT").valid()){
			$("#mainFormCCTVIT").remove(".help-block");
			$("#mainFormCCTVIT > .form-group").removeClass('has-error');
			$.ajax({
				url: url,
	...
	function ajax_load_table(filter){
		table.ajax.url('<?php echo base_url()."hanggar/monevcctvit/ajax_list/"?>'+filter).load();
	}
</script>